...
def get_data(column_letters: str, companies_filter=None, compare_function='IN', ignore_empty=True):
    ...
    xls = pandas.read_excel(
        './input/дисертація зведена таблиця без додатків_reduced_ua.xlsx',
        skiprows=2,
    ...
    for i, row in xls.iterrows():
        ...
        if r_point:
            ...
            elif compare_function == 'IN' and r_name in companies_filter:
                ids.append(r_id)
                names.append(r_name)
                points.append(r_point)
            elif compare_function == 'NOT' and r_name not in companies_filter:
                ids.append(r_id)
                names.append(r_name)
                points.append(r_point)
